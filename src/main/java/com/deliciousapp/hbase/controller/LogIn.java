...
public class LogIn {
    ...
    @POST
    @Path("/authorize_login")
    @Produces({"application/json"})
    @Consumes({"application/json"})
    @RequestMapping(value = "/authorize_login", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<String> authorizeLogin(
            @Valid @RequestBody EmailLogIn request) throws IOException, IllegalAccessException {
        long startMillis = System.currentTimeMillis();
        log.info("Start Timestamp: " + LocalDateTime.now());
        log.info("Request:\n");
        log.info(new GsonBuilder().setPrettyPrinting().create().toJson(request));
        Response response = profileService.authorizeLogin(request);
